...
class AnalyzerNetworkBase(AnalyzerBase):
    ...
    * allows :func:`_create_analysis` to return tensors
    ...
    def __init__(self, model,
                 neuron_selection_mode="replace",
                 allow_lambda_layers=False,
                 **kwargs):
        if neuron_selection_mode not in ["max_activation", "index", "all", "replace"]:
        ...
        self._special_helper_layers = []  #  the layers that helps to select the neuron to analyze, either K.max or tf.gather_nd
    ...
    def _prepare_model(self, model):
        ...
        if model_output[0].shape[-1] == 512:
            shape = (14,14,512)
        if model_output[0].shape[-1] == 2048:
            shape = (7,7,2048)
        ...
        if neuron_selection_mode == "max_activation":
            ...
            self._special_helper_layers.append(l)
        elif neuron_selection_mode == "replace":
            analysis_inputs.append(keras.layers.Input(shape=shape))
        else:
            raise NotImplementedError()
    ...
    def create_analyzer_model(self):
        ...
        # print(self._analyzer_model.summary())
    ...
    * :func:`_head_mapping` defines how the outputs of the model
    ...
    def _create_analysis(self, model, stop_analysis_at_tensors=[]):
        ...
        if return_all_reversed_tensors:
            ...
            mapping = {i: v["id"] for i, v in enumerate(values)}  # v["id"] = (nid, i)
