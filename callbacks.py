...
class LogMetrics(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        self._inference = BasicInference(self.model, self._dataset_provider, self._dataset_provider)
        self._old_logs = {}
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        print('Memory callback training before: ', info.total, psutil.Process(os.getpid()).memory_info().rss)
        # new_logs.update({k: np.float32(v) for k, v
        #                  in self._inference.evaluate_training_set().items()})
        #
        # info = psutil.virtual_memory()
        # print('Memory callback training: ', info.total, psutil.Process(os.getpid()).memory_info().rss)
        new_logs.update({'val_' + k: np.float32(v) for k, v
                         in self._inference.evaluate_test_set().items()})

        info = psutil.virtual_memory()
        print('Memory call back val: ', info.total, psutil.Process(os.getpid()).memory_info().rss)
